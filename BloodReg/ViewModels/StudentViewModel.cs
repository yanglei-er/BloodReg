...
namespace BloodReg.ViewModels
{
    public partial class StudentViewModel : ObservableObject, INavigationAware
    {
        ...
        [ObservableProperty]
        private ObservableCollection<Student> _todoList = [];

        [ObservableProperty]
        private int _todoListSelectIndex = -1;
        ...
        [ObservableProperty]
        private int _displayIndex = SettingsHelper.GetInt("StudentDisplayIndex");
        ...
        public StudentViewModel(ISqlSugarClient _db, ISnackbarService _snackbarService, IContentDialogService contentDialogService)
        {
            db = _db;
        ...
        private async void AutoSuggest(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                ...
                {
                    DataGridItems = (await db.Queryable<Student>().Where(it => it.StudentId.Contains(value)).ToDataTableAsync()).DefaultView;
                }
                else
                {
                    DataGridItems = (await db.Queryable<Student>().Where(it => it.Name.Contains(value)).ToDataTableAsync()).DefaultView;
                }
        ...
        public async Task AddAsync()
        {
            ...
            {
                if (await IsStudentExistsAsync(StudentID))
                {
                    System.Media.SystemSounds.Asterisk.Play();
                ...
                {
                    TodoList.Remove(studentToRemove);
                }
            }
            else
            {
                TodoList.Add(new Student(Name, StudentID, DonationVolume, Clerk));
            }
            Name = string.Empty;
        ...
        public async ValueTask<bool> IsStudentExistsAsync(string id)
        {
            return await db.Queryable<Student>().AnyAsync(it => it.StudentId == id);
        }

        public async ValueTask<bool> Update(Student student, string oldId)
        {
            if (student.StudentId != oldId && await IsStudentExistsAsync(student.StudentId))
            {
                System.Media.SystemSounds.Asterisk.Play();
            ...
            {
                await db.Ado.ExecuteCommandAsync("UPDATE Student SET Name=@name, StudentId=@id, DonationVolume=@volume, Clerk=@clerk WHERE StudentId=@oldid",
                new List<SugarParameter>() { new("@name", student.Name), new("@id", student.StudentId), new("@volume", student.DonationVolume), new("@clerk", student.Clerk), new("oldid", oldId) });
                return true;
            }
        }

        [RelayCommand]
        private async Task DelAsync(DataRowView selectedItem)
        {
            System.Media.SystemSounds.Asterisk.Play();
            ContentDialogResult result = await contentDialogService.ShowSimpleDialogAsync(new SimpleContentDialogCreateOptions()
            {
                Title = "删除",
                Content = $"是否删除学生【{(string)selectedItem[0]}】 ，此操作不可撤销！",
                PrimaryButtonText = "是",
                CloseButtonText = "否",
            });
        ...
        [RelayCommand]
        private void DelTodo(int SelectedIndex)
        {
            TodoList.RemoveAt(SelectedIndex);
        }

        partial void OnTodoListSelectIndexChanged(int value)
        {
            Name = TodoList[value].Name;
            StudentID = TodoList[value].StudentId;
            DonationVolume = TodoList[value].DonationVolume;
