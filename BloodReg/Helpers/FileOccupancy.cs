...
namespace BloodReg.Helpers
{
    ...
    {
        public static async Task<string> GetDirectorySizeAsync(string path)
        {
            if (Directory.Exists(path))
            {
                DirectoryInfo folder = new(path);
                long size = await Task.Run(() => FolderSize(folder));
        ...
        public static async Task<string> GetFileSizeAsync(string path)
        {
            if (File.Exists(path))
            {
                FileInfo fileInfo = new(path);
                long length = await Task.Run(() => fileInfo.Length);
